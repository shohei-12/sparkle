...
import { getIsSignedIn, getUserId, getUserProfile, getTheme } from '../../re-ducks/users/selectors';
import { signOut, deleteUser, toggleTheme } from '../../re-ducks/users/operations';
...
const DrawerMenu = () => {
  ...
  const dispatchToggleLightTheme = useCallback(() => {
    isSignedIn ? dispatch(toggleTheme('light')) : dispatch(toggleThemeAction({ theme: 'light' }));
    if (window.innerWidth < 960) {
      handleDrawerToggle();
    }
  ...
  const dispatchToggleDarkTheme = useCallback(() => {
    isSignedIn ? dispatch(toggleTheme('dark')) : dispatch(toggleThemeAction({ theme: 'dark' }));
    if (window.innerWidth < 960) {
      handleDrawerToggle();
    }
  }, [dispatch, handleDrawerToggle, isSignedIn]);
  ...
  const signInList = [
    {
      text: 'マイページ',
      icon: <img className={classes.img} src={profile ? profile : NoProfile} alt="プロフィール画像" />,
      path: `/users/${uid}`,
    },
  ];
