...
const UserEdit: React.FC = () => {
  ...
  const inputSelfIntroduction = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const inputCurrentPassword = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const updateUser = () => {
    ...
    axios({
      ...
      .catch((error) => {
        ...
        if (errorData.errors.full_messages.includes('Email has already been taken')) {
          setDuplicateEmail(email);
        }
        if (errorData.errors.full_messages.includes('Current password is invalid')) {
    ...
    <div className="wrap">
      ...
      <TextInput
        ...
        inputRef={register({
          ...
          validate: (value) => value !== duplicateEmail || 'このメールアドレスはすでに存在します。',
        })}
        error={Boolean(errors.email) || (Boolean(duplicateEmail) && email === duplicateEmail)}
        helperText={
          (errors.email && errors.email.message) ||
          (duplicateEmail && email === duplicateEmail && 'このメールアドレスはすでに存在します。')
        }
        inputProps={
          uid === '1'
            ? {
      ...
      <TextInput
        ...
        inputRef={register({
          ...
          pattern: {
            value: /^[\w-]+$/,
            message: '半角英数字、ハイフン(-)、アンダーバー(_)のみ利用可能です。',
          },
        })}
