...
const UserDetails: React.FC = () => {
  ...
  const over = useCallback((n: number) => {
    document.getElementById(`unfollow-btn${n}`)!.firstElementChild!.innerHTML = 'フォロー解除';
  }, []);

  const leave = useCallback((n: number) => {
    document.getElementById(`unfollow-btn${n}`)!.firstElementChild!.innerHTML = 'フォロー中';
  }, []);
    ...
    if (window.location.pathname === `/users/${uid}`) {
      ...
      axios
        ...
        .then((res) => {
          ...
          const scrollYLikeRecords = Number(localStorage.getItem('scrollY-like_records'));
        ...
        <>
          <div className={classes.flex}>
            <div className={classes.left}>
              ...
              <img className={classes.profile} src={profile ? profile : NoProfile} alt="プロフィール画像" />
            </div>
            <div className={classes.right}>
              <p>{selfIntroduction}</p>
            </div>
          </div>
          {currentUserId === uid && <SecondaryButton text="ユーザー情報を編集する" onClick={goUserEditPage} />}
          {currentUserId !== uid && (
            <>
              {following ? (
                <Button
                ...
                <Button id="follow-btn-1" variant="outlined" color="primary" onClick={dispatchFollow}>
                  フォロー
                </Button>
              )}
            </>
